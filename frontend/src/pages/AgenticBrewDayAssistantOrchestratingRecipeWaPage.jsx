...
// Feature: Agentic brew-day assistant orchestrating recipe, water chem, yeast, and CIP scheduling
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticBrewDayAssistantOrchestratingRecipeWaPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-brew-day-assistant-orchestrating-recipe-wa/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic brew-day assistant orchestrating recipe, water chem, yeast, and CIP scheduling</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-brew-day-assistant-orchestrating-recipe-wa/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
