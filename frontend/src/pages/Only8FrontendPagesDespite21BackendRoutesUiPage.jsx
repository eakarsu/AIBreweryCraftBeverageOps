...
export default function Only8FrontendPagesDespite21BackendRoutesUiPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-only-8-frontend-pages-despite-21-backend-routes-ui/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 8 frontend pages despite 21 backend routes — UI gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-8-frontend-pages-despite-21-backend-routes-ui/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
