...
// Feature: RAG over BJCP guidelines, in-house brew logs, and competition feedback
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverBjcpGuidelinesInHouseBrewLogsAndCoPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-bjcp-guidelines-in-house-brew-logs-and-co/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over BJCP guidelines, in-house brew logs, and competition feedback</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-bjcp-guidelines-in-house-brew-logs-and-co/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
