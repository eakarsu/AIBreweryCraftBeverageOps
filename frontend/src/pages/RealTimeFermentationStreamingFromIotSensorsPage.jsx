...
// Feature: Real-time fermentation streaming from IoT sensors with anomaly alerts
import React, { useState } from 'react';
import api from '../services/api';

export default function RealTimeFermentationStreamingFromIotSensorsPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-real-time-fermentation-streaming-from-iot-sensors-/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time fermentation streaming from IoT sensors with anomaly alerts</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-fermentation-streaming-from-iot-sensors-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
