...
const ALLOWED_EVENTS = [
  ...
  'lab_result.recorded', 'lab_result.out_of_spec',
  'fermentation.alert', 'tank.cip_due',
  'inventory.low', 'distribution.shipped', 'event.scheduled'
];
...
async function ensureTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS webhooks (
      ...
      user_id INTEGER,
      url TEXT NOT NULL,
      events TEXT[] NOT NULL DEFAULT ARRAY['batch.completed']::text[],
      secret VARCHAR(255),
...
router.post('/', async (req, res) => {
  try {
    ...
    const incoming = Array.isArray(events) && events.length > 0 ? events : ['batch.completed'];
...
router.post('/:id/test', async (req, res) => {
  try {
    ...
    const payload = {
      ...
      data: { message: 'Test webhook from AIBreweryCraftBeverageOps' }
    };
